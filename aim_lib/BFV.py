from aim_lib import MemAccess
from aim_lib.MemAccess import *
from aim_lib.PointerManager import PointerManager
from aim_lib import offsets
...
def GetEncKey(pHandle, typeinfo):
    ...
    api._cache_en = cache_en
    print(f"[+] Typeinfo: {hex(typeinfo)} Encryption Key: {hex(keystore[typeinfo])}")
    return keystore[typeinfo]
...
def DebugDrawVec3(canvas, vec, text_x, text_y, title="", font_size=15, color="lavender", anchor='nw'):
    text = DebugPrintVec4(vec)
    if title != "":
        text = title + "\n" + text
    canvas.create_text(text_x, text_y, fill=color, font=("Times 20 italic bold", font_size), anchor=anchor,
                       text=text)

def DebugDrawMatrix(canvas, matrix, text_x, text_y, title="", font_size=15, color="lavender", anchor='nw'):
    text = DebugPrintMatrix(matrix)
    if title != "":
        text = title + "\n" + text
    canvas.create_text(text_x, text_y, fill=color, font=("Times 20 italic bold", font_size), anchor=anchor,
                       text=text)
...
def process(pHandle, cnt, aim_location):
    ...
    for Soldier in GetEntityList(pHandle, offsets.ClientSoldierEntity, 0xF0):
        ...
        ##################################################################################
        main_spine = [bones["Hips"], bones["Spine"], bones["Spine1"], bones["Spine2"], bones["Neck"], bones["Neck1"],
                      bones["Head"], bones["HeadEnd"]]
        arm_r = [bones["Spine2"], bones["RightShoulder"], bones["RightArm"], bones["RightForeArm"], bones["RightHand"]]
        arm_l = [bones["Spine2"], bones["LeftShoulder"], bones["LeftArm"], bones["LeftForeArm"], bones["LeftHand"]]

        aim_main_spine = []
        aim_arm_r = []
        aim_arm_l = []
        for i in main_spine:
            aim_main_spine.append(mem[Soldier](0x6e0)(0x20).read_vec4(i * 0x20))
        for i in arm_r:
            aim_arm_r.append(mem[Soldier](0x6e0)(0x20).read_vec4(i * 0x20))
        for i in arm_l:
            aim_arm_l.append(mem[Soldier](0x6e0)(0x20).read_vec4(i * 0x20))
        #####################################################################################
        ...
        SoldierData.joint = [aim_main_spine, aim_arm_r, aim_arm_l]
